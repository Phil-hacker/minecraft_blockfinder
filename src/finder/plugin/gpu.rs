use std::{borrow::Cow, mem::size_of, num::NonZeroU64, sync::mpsc::channel, time::{Duration, Instant}};
...
use crate::{
    ...
    finder::{chunk::create_box, util::spiral}, AppState,
};
...
fn update_label(mut label: Query<&mut Text, With<SearchedChunksLabel>>, finder_status: Res<FinderStatus>) {
    let mut formatter = human_format::Formatter::new();
    let mut scale = Scales::new();
    scale.with_suffixes(vec!["thousand".into(),"million".into(),"billion".into(),"trillion".into(),"quadrillion".into(),"quintillion".into(),"sextillion".into(),"septillion".into()]);
    formatter.with_scales(scale);
    match finder_status.as_ref() {
        FinderStatus::WaitingForJob => {},
        FinderStatus::Running { blocks, start_time } => {
            *label.single_mut().as_mut() = Text::from_section(format!("{} blocks searched\n{} seconds elapsed", formatter.format(*blocks as f64), start_time.elapsed().as_secs()), TextStyle::default());
        },
        FinderStatus::Finished { searched_blocks, pos, time } => {
            *label.single_mut().as_mut() = Text::from_section(format!("{} blocks searched\n{} seconds elapsed\n Found at: {}", formatter.format(*searched_blocks as f64), time.as_secs(), pos), TextStyle::default());
        },
    }
...
impl FromWorld for FindShaderData {
    fn from_world(world: &mut World) -> Self {
        ...
        let result_cpu = render_device.create_buffer(&BufferDescriptor { label: None, size: UVec3::min_size().into() , usage: BufferUsages::MAP_READ | BufferUsages::COPY_DST, mapped_at_creation: false });
    ...
    fn run<'w>(
        ...
        match self.0 {
            ...
            FindNodeState::WaitingForGPU | FindNodeState::ReadingData => {
                ...
                pass.dispatch_workgroups((CHUNK_SIZE - CHUNK_MARGIN) as u32, (WORLD_HEIGHT - GRID_SIZE.1) as u32, (CHUNK_SIZE - CHUNK_MARGIN) as u32);
                drop(pass);
        ...
        match self.0 {
            ...
            FindNodeState::WaitingForTask => {
                if let Some(job) = world.get_resource::<FinderJob>() {
                    let grid = unsafe {std::mem::transmute::<Box<[u8; GRID_SIZE.0 * GRID_SIZE.1 * GRID_SIZE.2 ]>, Box<[u32; GRID_SIZE.0 * GRID_SIZE.1 * GRID_SIZE.2 / 4 ]>>(job.0.clone())};
                    ...
                    world.get_resource_mut::<FinderStatus>().map(|mut finder_status| {
                        *finder_status.as_mut() = FinderStatus::Running { blocks: 0, start_time: Instant::now() };
                    });
            ...
            FindNodeState::ReadingData => {
                world.resource_scope(|world, pipeline: Mut<FindShaderData> |{
                    ...
                    buffer_slice.map_async(bevy::render::render_resource::MapMode::Read, move |v| match v {
                        Ok(_) => sender.send(()).unwrap(),
                        Err(err) => panic!("couldn't read data from gpu: {err:?}"),
                    });
                    world.get_resource_mut::<FinderStatus>().map(|mut finder_status| {
                        if let FinderStatus::Running { blocks, start_time: _ } = finder_status.as_mut() {
                            *blocks = *blocks + ((CHUNK_SIZE - CHUNK_MARGIN) * (CHUNK_SIZE - CHUNK_MARGIN) * WORLD_HEIGHT) as u64
                        }
                    });
                    let render_device = world.resource::<RenderDevice>();
                    ...
                    if data == UVec3::splat(u32::MAX) {
                        ...
                        let (x, y) = (x * (CHUNK_SIZE - CHUNK_MARGIN) as i32, y * (CHUNK_SIZE - CHUNK_MARGIN) as i32);
                        ...
                        world.get_resource_mut::<FinderStatus>().map(|mut finder_status| {
                            ...
                            if let FinderStatus::Running { blocks: v, start_time } = finder_status.as_ref() {
                                blocks = *v;
                                duration = start_time.elapsed();
                            }
                            *finder_status.as_mut() = FinderStatus::Finished { searched_blocks: blocks, pos: data, time: duration }
                        });
                    }
                    drop(buffer_view);
                    buffer.unmap();
...
fn set_position(world: &mut World, chunk_index: u32) {
    world.resource_scope(|world, mut pipeline: Mut<FindShaderData>| {
        ...
        let pos = (x * (CHUNK_SIZE - CHUNK_MARGIN) as i32, y * (CHUNK_SIZE - CHUNK_MARGIN) as i32).into();
        info!("{pos}");
